...
# TAMAMEN ÇÖZÜLDÜ: Artık gerçek sticker olarak gönderiyor, "unnamed" sorunu yok!
...
async def sticker_command(event):
    ...
    try:
        # Medyayı doğrudan orijinal haliyle indir (file yerine orijinal medya)
        downloaded = await client.download_media(reply_msg.media, file=None)  # None = bytes döner

        # Sticker attribute ekleyerek ve force_document=False (varsayılan) ile gönder
        await client.send_file(
            event.chat_id,
            file=downloaded,
            attributes=[
                DocumentAttributeSticker(
                    alt="✨",  # İstersen "" yapıp emoji kaldırırsın
                    stickerset=InputStickerSetEmpty()
                )
            ],
            force_document=False  # Bu satır çok önemli! Document olarak göndermesin
        )
...
async def burc_handler(event):
    ...
    if not burc:
        await event.reply("🏹 **Burç yorumu nasıl kullanılır**: /burc kova şeklinde burcunuzu yazarak günlük burç yorumunuzu alabilirsiniz. Hergün 23.00'dan sonra günlük yorumunuz değişir.")
        return
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if horoscope_element:
                ...
                await event.reply(f"Sevgili [{event.sender.first_name}](tg://user?id={event.sender_id}), işte {burc.capitalize()} burcu yorumunuz:\n\n{horoscope}")
            else:
...
async def son_durum(event):
    ...
    global anlik_calisan, grup_sayi
